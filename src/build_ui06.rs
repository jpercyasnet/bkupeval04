...
//----------------- target directory button start -----------------------------------
    cdirectory_o_button.connect_clicked(glib::clone!(@weak window, @weak cdirectory_o_combobox, @weak ctarget_entry,  @weak messageval_label => move|_| {
...
//----------------- get directory list button start -----------------------------------
    hexget_button.connect_clicked(glib::clone!(@weak hdirectory1_combobox, @weak progress_progressbar, @weak messageval_label  => move|_| {
// files must be in utf-8 format
// linux command file -i filename will show format
// linux command iconv -f format -t UTF-8 filename -o outputfile    will convert file to UTF-8   
        let mut bolok = true;
